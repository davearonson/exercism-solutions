defmodule AllYourBase do
  ...
  def convert( digits,  base_a,  base_b) do
    with {:ok, value} <- read_digits_in_base(digits, base_a, 0),
         new_digits   <- write_digits_in_base(value, base_b, []),
         final_digits <- fix_digit_list(new_digits)
    do
      final_digits
    else
      err -> err
  ...
  defp write_digits_in_base(    0, _base, acc), do: acc
  defp write_digits_in_base(value,  base, acc)  do
    write_digits_in_base(div(value, base), base, [rem(value, base)|acc])
  end

  defp fix_digit_list([]    ), do: [0]
  defp fix_digit_list(digits), do: digits
